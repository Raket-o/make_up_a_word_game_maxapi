...
2. Копируем pip.conf в папку venv
3. Активируем окружение ```venv\Scripts\activate.bat```
...
6. Запускаем командой из терминала ```python maxapi_bot/main.py```

### Запуск на Linux (при установленном python):
Открываем терминал, переходим в корневую папку с ботом
1. Создаём окружение ```python3 -m venv venv```   
2. Копируем pip.conf в папку venv
3. Активируем окружение source ```source venv/bin/activate```
4. Устанавливаем зависимости ```pip install -r requirements.txt```
5. Переименовываем файл .env.template в .env
6. Заполняем поля токен-бота и URL веб приложения
7. Запускаем командой из терминала ```python3 maxapi_bot/main.py```
